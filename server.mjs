...
async function initDatabase() {
  ...
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS school_exams (
      exam_id TEXT PRIMARY KEY, school_id TEXT NOT NULL REFERENCES schools(school_id) ON DELETE CASCADE,
      term_id TEXT REFERENCES school_terms(term_id) ON DELETE SET NULL, class_id TEXT NOT NULL REFERENCES school_classes(class_id) ON DELETE CASCADE,
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
          ...
          body: JSON.stringify({
            ...
            generationConfig: {
              thinkingConfig: {
                thinkingLevel: (mode === 'notes' || mode === 'summary' || mode === 'practice') ? 'low' : 'medium'
              }
            }
          })
        });
